...
- Direct tensor API access (no CPU→GPU transfers)
...
1. `OnPlaybackTick` → Clock source
2. `ROS2PublishJointState` → Actual publisher
3. `targetPrim` → Robot to publish
...
```
ROS 2 topic → ROS2SubscribeJointState → ArticulationController → Robot
```
...
- **Impact:** 10 FPS → 15-20 FPS (50-100% improvement)
...
- **Impact:** 10 FPS → 100+ FPS (900%+ improvement)
...
- Direct tensor operations (no CPU↔GPU transfers)
...
- **Impact:** 10 FPS → 12-15 FPS (20-50% improvement)
...
- 7/7 correct: 🏆 **Expert** - Ready for advanced Isaac Sim development
- 5-6/7 correct: ✅ **Proficient** - Good understanding, minor gaps
- 3-4/7 correct: ⚠️ **Developing** - Review key concepts (GPU physics, USD, ROS 2)
- 0-2/7 correct: 📖 **Needs Review** - Re-read Chapter 06 sections
...
|----------|-------------|----------------|
| Q1 | GPU Requirements | Prerequisites → System Requirements |
| Q2 | USD Format | Key Concepts → USD Format |
| Q3 | PhysX GPU Physics | Enabling GPU Physics |
| Q4 | ROS 2 Bridge | ROS 2 Bridge Setup |
| Q5 | Synthetic Data | Advanced → Synthetic Data Generation |
| Q6 | Performance | Common Gotchas → Slow Simulation |
...
1. ✅ **Score ≥ 80%:** Proceed to Chapter 10 (VLA Models)
2. ⚠️ **Score < 80%:** Review Chapter 06 sections and retry quiz
3. 🎯 **Hands-on:** Run code examples in `code-examples/isaac-sim/`
4. 🚀 **Advanced:** Explore Isaac Gym for RL training
