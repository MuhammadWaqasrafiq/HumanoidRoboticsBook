...
- ✅ Works without dedicated GPU (can use integrated graphics)
- ✅ Actively developed (Gazebo Classic is deprecated)
- ✅ Good performance on moderate hardware (16 GB RAM sufficient)
- ✅ ROS 2 native integration via `ros_gz_bridge`
- ✅ Modern architecture with DART physics (good for humanoids)
...
|-----------|---------|---------|------------------|
| Gazebo Classic | None | 4 GB | ✅ |
| Ignition Gazebo | Integrated OK | 8 GB | ✅ |
| Isaac Sim | **RTX 2070+** | 32 GB | ❌ (needs dedicated GPU) |
...
- ✅ **Excellent articulated body dynamics** - designed for robots with many joints
- ✅ **Stable constraint solver** - handles complex kinematic chains reliably
- ✅ **Accurate contact modeling** - doesn't let robots fall through floor
- ✅ **Fast inverse dynamics** - efficient for walking/balancing algorithms

**ODE (A) problems:**

```
❌ Poor contact dynamics - humanoids can fall through floor
...
```
⚠️ Contact friction less accurate than DART
...
- RTF = 1.0 → Simulation matches real-time (perfect)
- RTF = 2.0 → Simulation runs 2x faster (very good!)
- **RTF = 0.6 → Simulation runs at 60% of real-time speed (too slow)**
...
# CPU (Ignition Gazebo):
# 200 Hz physics × 1 robot = 200 steps/sec
# 10M steps / 200 = 50,000 seconds = 13.9 hours

# GPU (Isaac Sim + Isaac Gym):
# 1000 Hz physics × 100 parallel robots = 100,000 steps/sec
...
```
Gazebo Classic rendering → Real camera
...
|----------|----------------------|----------|-------------------|----------|
| **0.1s (10 Hz)** | 10 steps | Poor | Objects pass through | ❌ Not recommended |
...
|-------|-------------------|-----------|------------|
| **Gazebo Classic** (50 worlds) | ~3,000 steps/sec | 25 GB (0.5 GB × 50) | High (manage 50 processes) |
...
# Result:
# - 1000 Hz physics × 50 robots = 50,000 physics steps/sec
...
- ❌ High RAM overhead (separate processes)
- ❌ No parallel physics (each world runs independently)
- ❌ Complex orchestration (manage 50 separate processes)

**B (Ignition Gazebo, 50 in 1 world):**

- ✅ Better than separate worlds (shared rendering, one process)
- ⚠️ CPU-bound - scales linearly (2x robots = 2x slower)
- ⚠️ Not optimized for RL training (no Tensor API)

**D (50 computers):**

- ❌ Extremely complex setup (network, data sync)
- ❌ High cost (50 machines)
- ❌ Orchestration overhead
- ✅ Only makes sense for very large scales (1000+ robots) if you already have cluster
